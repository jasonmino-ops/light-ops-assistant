...
export type EffectBoundary = 'NOT_CROSSED' | 'CROSSING_UNKNOWN' | 'CROSSED'
...
export class PrintDeliveryError extends Error {
  constructor(
    ...
    public readonly effectBoundary: Exclude<EffectBoundary, 'CROSSED'>,
...
/** Existing fixed Windows RAW queue transport. Reliability is handled above it. */
export class WindowsQueueTransport {
  ...
  async deliver(commandStream: Uint8Array, documentName: string): Promise<PrintDelivery> {
    if (this.busy) throw new PrintDeliveryError('TRAY_BUSY', 'NOT_CROSSED')
    if ((this.options.platform ?? process.platform) !== 'win32') {
      throw new PrintDeliveryError('WINDOWS_REQUIRED', 'NOT_CROSSED')
    }
    if (
      !(commandStream instanceof Uint8Array)
      || commandStream.byteLength === 0
      || commandStream.byteLength > ESHOP_TRAY_MAX_COMMAND_BYTES
    ) throw new PrintDeliveryError('INVALID_COMMAND_STREAM', 'NOT_CROSSED')
  ...
  private writeWithPowerShell(commandStream: Uint8Array, documentName: string): Promise<number> {
    ...
    try {
      ...
      throw new PrintDeliveryError('PRINT_DELIVERY_FAILED', 'NOT_CROSSED', { cause })
    }
    ...
    return new Promise<number>((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        child.kill()
        finish(() => reject(new PrintDeliveryError('PRINT_TIMEOUT', 'CROSSING_UNKNOWN')))
      }, timeoutMs)
      ...
      child.once('error', (cause) => {
        finish(() => reject(new PrintDeliveryError('PRINT_DELIVERY_FAILED', 'CROSSING_UNKNOWN', { cause })))
      })
      child.once('close', (code) => {
        finish(() => {
          if (code !== 0) {
            reject(new PrintDeliveryError('PRINT_DELIVERY_FAILED', 'CROSSING_UNKNOWN', {
          ...
          try {
            ...
            reject(new PrintDeliveryError('PRINT_DELIVERY_FAILED', 'CROSSING_UNKNOWN', { cause }))
          }
        })
      })
