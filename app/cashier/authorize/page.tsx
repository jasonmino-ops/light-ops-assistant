...
export default function CashierAuthorizePage() {
  ...
  useEffect(() => {
    ...
    apiFetch(`/api/cashier/device-authorization/${encodeURIComponent(requestId)}`, { cache: 'no-store' })
      .then(async (res) => {
  ...
  async function confirm() {
    ...
    setError('')
    try {
      const res = await apiFetch(`/api/cashier/device-authorization/${encodeURIComponent(requestId)}`, {
        method: 'POST',
