...
export async function POST(req: NextRequest) {
  return withComputerClientApiError(async () => {
    ...
    const consumed = await prisma.$transaction(
      async (tx) => {
        const candidate = await tx.computerBrowserLaunchTicket.findUnique({
          where: { ticketHash },
          select: { bindingId: true, usedAt: true, expiresAt: true },
        })
        if (!candidate || candidate.usedAt || candidate.expiresAt.getTime() <= now.getTime()) {
          return null
        }

        // 与 OWNER 停用操作共用同一 binding 行锁：兑换先完成时，停用随后能看到并撤销
        // 新 Session；停用先完成时，兑换在锁释放后读取到 disabledAt 并拒绝。
        await tx.$queryRaw`SELECT "id" FROM "ComputerBinding" WHERE "id" = ${candidate.bindingId} FOR UPDATE`
