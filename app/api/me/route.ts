...
export async function GET(req: NextRequest) {
  ...
  const [tenant, store] = await Promise.all([
    ...
    ctx.storeId
      ? prisma.store.findUnique({ where: { id: ctx.storeId }, select: { id: true, name: true, code: true, checkoutMode: true, currencyCode: true } })
      ...
      : prisma.store.findFirst({
          ...
          select: { id: true, name: true, code: true, checkoutMode: true, currencyCode: true },
        }),
  ])
