...
import { browserPosReturnTo } from '@/lib/browser-pos-entry'
...
export default function DesktopModePage() {
  ...
  const [entryState, setEntryState] = useState<'checking' | 'select' | 'binding'>('checking')
  ...
  useEffect(() => {
    ...
    if (sc && (isNativeDesktopRuntime() || getPosDeviceToken(sc))) {
      window.location.replace(browserPosReturnTo(sc, nextLang))
      return
    }
    setEntryState(sc ? 'binding' : 'select')
  }, [])
  ...
  if (entryState === 'checking') return null

  if (entryState === 'binding') {
    const returnTo = browserPosReturnTo(storeCode, lang)
    return (
      <main style={s.page}>
        <section style={s.panel}>
          ...
          <h1 style={s.title}>{t.bindingTitle}</h1>
          <p style={s.desc}>{t.bindingDesc}</p>
          <div style={s.storeBadge}>{t.storeLabel}{storeCode}</div>
          <a href={`/cashier/authorize?returnTo=${encodeURIComponent(returnTo)}`} style={{ ...s.card, ...s.cardActive, marginTop: 20, textDecoration: 'none' }}>
            <div style={s.icon}>🔐</div>
            <div style={s.cardBody}>
              <div style={s.cardTitle}>{t.bindingAction}</div>
              <div style={s.cardDesc}>{t.bindingNoLink}</div>
            </div>
          </a>
        </section>
      </main>
...
const desktopCopy: Record<DesktopLang, Record<string, string>> = {
  zh: {
    ...
    posAction: '进入收银台',
    bindingTitle: '绑定当前电脑',
    bindingDesc: '这条旧门店链接不能授权收银。请使用 Telegram 老板端生成的 10 分钟一次性电脑链接，在本机完成绑定。',
    bindingAction: '绑定当前电脑',
    bindingNoLink: '没有有效链接？请从 Telegram 老板端重新生成电脑链接。',
    displayTitle: '顾客显示屏',
  ...
  en: {
    ...
    posAction: 'Open POS',
    bindingTitle: 'Bind this computer',
    bindingDesc: 'This legacy store link cannot authorize a cashier. Use a 10-minute computer link generated by the Telegram owner on this computer.',
    bindingAction: 'Bind this computer',
    bindingNoLink: 'No valid link? Ask the Telegram owner to generate a new computer link.',
  ...
  km: {
    ...
    posAction: 'ចូលបញ្ជរ',
    bindingTitle: 'ភ្ជាប់កុំព្យូទ័រនេះ',
    bindingDesc: 'តំណហាងចាស់នេះមិនអាចផ្តល់សិទ្ធិបញ្ជរបានទេ។ សូមប្រើតំណកុំព្យូទ័រ ១០ នាទីពីម្ចាស់ Telegram លើកុំព្យូទ័រនេះ។',
    bindingAction: 'ភ្ជាប់កុំព្យូទ័រនេះ',
    bindingNoLink: 'គ្មានតំណត្រឹមត្រូវ? សូមឲ្យម្ចាស់ Telegram បង្កើតតំណថ្មី។',
