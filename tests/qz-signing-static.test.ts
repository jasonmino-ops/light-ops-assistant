...
assert.doesNotMatch(
  signingRoute,
  /isQzSigningStoreAllowed|QZ_SIGN_STORE_FORBIDDEN/,
  'a valid Browser POS Device Session must not be restricted by the historical signing store allowlist',
)
...
assert.match(server, /computerBrowserLaunchTicket\.findFirst/)
assert.match(server, /browserPosDeviceId:\s*payload\.browserPosSessionId/)
assert.match(server, /binding:\s*\{\s*tenantId:\s*verified\.tenantId,\s*storeId:\s*verified\.storeId/)
