...
assert.match(cashier, /function closeSaleResultOverlay\(\) \{\s*if \(isReceiptPrintChainActive \|\| receiptPrintLockedRef\.current\) return/, 'background dismissal must be blocked while the chain is active')
assert.match(cashier, /function handleContinueSale\(\) \{\s*if \(isReceiptPrintChainActive \|\| receiptPrintLockedRef\.current\) return/, 'continue sale must be blocked while the chain is active')
