...
assert.match(cashierLaunchPage, /new URLSearchParams\(\{ storeCode: body\.storeCode, mode: 'pos' \}\)/, 'the final Desktop POS URL must use the consumed binding store and mode=pos')
assert.match(cashierLaunchPage, /window\.location\.replace\(`\/desktop\/pos\?\$\{nextParams\.toString\(\)\}`\)/, 'the official launch must enter the Desktop POS device context')
