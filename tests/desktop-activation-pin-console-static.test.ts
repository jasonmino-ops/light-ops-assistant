...
assert.match(opsRoute, /getFkBackedOpsAdminIdentity/, 'ops PIN API must reject synthetic identities before persistence')
assert.match(opsRoute, /OPS_ADMIN_IDENTITY_REQUIRED/, 'ops PIN API must return a stable identity error')
assert.match(opsRoute, /createdByOpsAdminId:\s*actor\.id/, 'ops PIN API must persist verified OpsAdmin creator id')
assert.match(opsRoute, /actorOpsAdminId:\s*actor\.id/, 'ops PIN API must persist verified OpsAdmin audit actor id')
assert.match(opsRoute, /operatorRole:\s*actor\.role/, 'ops PIN API must derive operator role from verified FK-backed identity')
