...
const ITEM_KEYS = ['name', 'quantity', 'unitPrice', 'lineTotal'] as const
...
export function validateRuntimeReceiptPayload(value: unknown): asserts value is RuntimeReceiptPayload {
  ...
  requireExactKeys(receipt as Record<string, unknown>, RECEIPT_REQUIRED_KEYS, RECEIPT_OPTIONAL_KEYS)
...
function validateItem(value: unknown): void {
  ...
  requireExactKeys(item as Record<string, unknown>, ITEM_KEYS)
...
function rejectUnsafeText(value: string): void {
  ...
  if (/\b(token|authorization|bearer)\b/i.test(value)) throw new Error('INVALID_PRINT_RECEIPT')
}
