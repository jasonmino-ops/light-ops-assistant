...
describe('Preload 与 shared 通道白名单同步（sandboxed preload 自包含约束）', () => {
  it('员工 preload 使用且仅使用 CART_PUBLISH 发送', () => {
    ...
    expect(employeePreloadSrc).toContain(`'${IPC_CHANNELS.DEPLOYMENT_GET_HEALTH}'`)
    expect(employeePreloadSrc).toContain(`'${IPC_CHANNELS.DEPLOYMENT_GET_SYSTEM_INFO}'`)
    expect(employeePreloadSrc).toContain(`'${IPC_CHANNELS.DEPLOYMENT_RETRY_CLOUD}'`)
    expect(employeePreloadSrc).toContain(`'${IPC_CHANNELS.DEPLOYMENT_RELOAD_BUSINESS}'`)
    expect(employeePreloadSrc).toContain(`'${IPC_CHANNELS.DEPLOYMENT_RECHECK_PROVIDER}'`)
    expect(employeePreloadSrc).toContain(`'${IPC_CHANNELS.DEPLOYMENT_RECHECK_DISPLAYS}'`)
    expect(employeePreloadSrc).toContain(`'${IPC_CHANNELS.DEPLOYMENT_OPEN_LOGS}'`)
    expect(employeePreloadSrc).toContain(`'${IPC_CHANNELS.DEPLOYMENT_EXPORT_DIAGNOSTICS}'`)
    expect(employeePreloadSrc).toContain(`'${IPC_CHANNELS.DEPLOYMENT_QUIT}'`)
    expect(employeePreloadSrc).toContain(`'${IPC_CHANNELS.DEPLOYMENT_RETURN_TO_ACTIVATION}'`)
  ...
  it('顾客 preload 使用 CART_APPLY 接收 + DISPLAY_READY 上报', () => {
    ...
    expect(customerPreloadSrc).not.toContain(`'${IPC_CHANNELS.DEPLOYMENT_GET_HEALTH}'`)
    expect(customerPreloadSrc).not.toContain(`'${IPC_CHANNELS.DEPLOYMENT_EXPORT_DIAGNOSTICS}'`)
  })
