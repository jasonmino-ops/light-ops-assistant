...
六阶段模型只适用于仍在本 Register 治理范围内的正式 Legacy 条目和候选治理对象。已移出 HRT 范围并转交其他治理域的历史记录，不进入六阶段生命周期。
...
| LEG-PRT-002 | 云打印链路（SW-AIOT） | 已归类移出（Cloud 远程执行服务边界） | N/A — OUT OF HRT SCOPE | 已确认（CLASSIFIED_OUT_OF_HRT_SCOPE / TRANSFERRED_TO_CLOUD_GOVERNANCE；产品决策：商户侧已隐藏/停用） | 无——不是 Milestone B HRT 迁移项 |
...
| 当前阶段 | **N/A — OUT OF HRT SCOPE**（已归类移出，保留为盘点记录） |
| 归类结论 | **CLASSIFIED_OUT_OF_HRT_SCOPE** |
| 治理转交 | **TRANSFERRED_TO_CLOUD_GOVERNANCE** |
...
本文件状态为 **FINAL FROZEN**（V1.0）。RC1 治理规则收口已完成，RC2 两轮 Codex 只读事实核实与事实回填已完成，创始人已确认 V1.0-RC2 正文并宣布本文件自 **2026-07-14** 起正式 FINAL FROZEN。

本次冻结的是治理规则、字段结构、首批治理对象与事实核实状态体系。条目事实字段、证据与迁移阶段仍按第 6 章作为运行数据维护；后续事实更新不产生 RC3。任何规则修改必须通过 supersede 机制执行，不得在本文件内直接改写冻结规则。
...
## ES-LEGACY-REGISTER-001 正式冻结决议

《Legacy Grandfathered Register V1》已完成治理规则起草、RC1 治理规则收口、五项拍板裁定、两轮 Codex 只读事实核实、RC2 事实回填与冻结候选审查。

正式文档编号：

ES-LEGACY-REGISTER-001
...
本文件自生效之日起，成为 E-Shop Store Operating System 内 Legacy Grandfathered 链路、入口、控制平面与迁移状态的正式治理登记册。
...
*ES-LEGACY-REGISTER-001 · Legacy Grandfathered Register V1 · V1.0 · FINAL FROZEN · 2026-07-14*
*上位文件：ES-CONST-001 / ES-STRAT-001 / ES-HRT-001（均 FINAL FROZEN）*
*版本演进：DRAFT → V1.0-RC1 → V1.0-RC2 → V1.0 FINAL FROZEN*
